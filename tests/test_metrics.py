...
class TestClassificationMetricsEdgeCases:
    ...
    def test_single_sample_positive(self):
        ...
        assert metrics["accuracy"] == 1.0
        assert metrics["recall"] == 1.0  # TP=1, FN=0
        assert metrics["tp"] == 1
        assert metrics["tn"] == 0
        # specificity = TN/(TN+FP) = 0/0, should be handled gracefully
        assert metrics["specificity"] == 0.0

    def test_single_sample_negative(self):
        """Single negative sample correctly predicted."""
        y_true = np.array([0])
        y_pred = np.array([0])
        ...
        assert metrics["accuracy"] == 1.0
        assert metrics["specificity"] == 1.0  # TN=1, FP=0
        assert metrics["tn"] == 1
        assert metrics["tp"] == 0
        # recall = TP/(TP+FN) = 0/0, should be handled gracefully
        assert metrics["recall"] == 0.0

    def test_only_positive_class_in_truth(self):
        """All ground truth is class 1, some predicted as 0."""
        ...
        assert metrics["recall"] == pytest.approx(2 / 4)  # TP=2, FN=2
        # No TN possible since no actual negatives
        assert metrics["tn"] == 0
        assert metrics["fp"] == 0
        # specificity = 0/0 handled as 0.0
        assert metrics["specificity"] == 0.0

    def test_only_negative_class_in_truth(self):
        """All ground truth is class 0, some predicted as 1."""
        ...
        assert metrics["accuracy"] == 0.5
        assert metrics["specificity"] == pytest.approx(2 / 4)  # TN=2, FP=2
        # No TP possible since no actual positives
        assert metrics["tp"] == 0
        assert metrics["fn"] == 0
        # recall = 0/0 handled as 0.0
        assert metrics["recall"] == 0.0

    def test_imbalanced_classes(self):
        """Heavily imbalanced classes (90% class 0, 10% class 1)."""
        y_true = np.array([0] * 90 + [1] * 10)
        y_pred = np.array([0] * 90 + [1] * 10)  # Perfect
